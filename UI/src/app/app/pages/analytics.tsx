...
export function AnalyticsPage() {
  ...
  const schemaDefinitionRef = useRef<HTMLDivElement>(null);
  ...
  const handleSidebarClick = (refTarget: React.RefObject<HTMLDivElement>, tabId?: string) => {
    if (tabId) {
      setActiveTab(tabId);
    }
    if (refTarget && refTarget.current) {
      refTarget.current.scrollIntoView({ behavior: 'smooth', block: 'center' });
      // Highlight effect with blue tint
      refTarget.current.style.transition = 'all 0.3s ease';
      refTarget.current.style.boxShadow = '0 0 0 3px rgba(123, 168, 196, 0.4)';
      refTarget.current.style.backgroundColor = 'rgba(123, 168, 196, 0.1)';
      setTimeout(() => {
        if (refTarget && refTarget.current) {
          refTarget.current.style.boxShadow = '';
          refTarget.current.style.backgroundColor = '';
        }
      }, 1500);
    }
  };
          ...
          <nav className="flex gap-1">
            {[
              ...
              <button
                ...
                onClick={() => {
                  if (tab.id === 'exports') {
                    handleSidebarClick(exportControlsRef, 'exports');
                  } else if (tab.id === 'schema') {
                    handleSidebarClick(schemaDefinitionRef, 'schema');
                  } else if (tab.id === 'audit') {
                    handleSidebarClick(auditReportRef, 'audit');
                  } else {
      ...
      <main className="max-w-[1800px] mx-auto px-8 py-8">
        <div className="grid grid-cols-12 gap-6">
          ...
          <div className="col-span-3 space-y-6">
            ...
            {/* Export Controls */}
            <div className="border border-white/10 rounded bg-white/5 backdrop-blur-sm p-5" ref={exportControlsRef}>
              <div className="text-xs text-white/70 mb-4 uppercase tracking-wider">
                Export Controls
              </div>
              ...
              <div className="space-y-2">
                ...
                <button className="w-full flex items-center justify-between px-3 py-2.5 bg-white/10 hover:bg-white/15 border border-white/20 rounded text-sm transition-colors" ref={schemaDefinitionRef}>
                  <span>Schema Definition</span>
                  <Download className="w-4 h-4" />
                </button>
                <button className="w-full flex items-center justify-between px-3 py-2.5 bg-white/10 hover:bg-white/15 border border-white/20 rounded text-sm transition-colors" ref={auditReportRef}>
